...
[[ "$(kubectl config current-context)" == kind-celln-m0 ]]
...
controller_pid=$!
jq --arg ns "$CELLN_PROOF_NAMESPACE" '{apiVersion:"sympozium.ai/v1alpha1",kind:"AgentRun",metadata:{name:"harness-proof",namespace:$ns},spec:{agentRef:"reference",agentId:"reference",sessionKey:"harness-proof",backend:"celln",task:.harness.task,model:{provider:"deepseek",model:.harness.model,authSecretRef:""},timeout:"180s",celln:{mote:.mote,tools:.tools,invocation:.invocation,lane:"agent",capabilities:(.capabilities|del(.timeoutMs)),harness:(.harness|del(.task,.model))}}}' "$CELLN_PROOF_REQUEST" >"$evidence/agentrun-request.json"
...
jq -e '.status.phase=="Succeeded" and (.status.cellnRequest|fromjson|.apiVersion=="celln.dev/v1alpha2" and .harness.model=="deepseek-chat") and (.status.cellnReceipt|fromjson|.apiVersion=="celln.dev/v1alpha2" and .phase=="succeeded")' "$evidence/agentrun-status.json" >/dev/null || { jq '.status' "$evidence/agentrun-status.json"; tail -50 "$evidence/controller.log"; exit 1; }
